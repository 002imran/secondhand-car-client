...
const AllUsers = () => {
    ...
    const { data: users = [], isLoading, refetch} = useQuery({
        queryKey: ['users'],
        queryFn: async() =>{
            const res = await fetch('https://secondhand-car-server-side.vercel.app/users');
    ...
    const handleMakeAdmin = id =>{
        fetch(`https://secondhand-car-server-side.vercel.app/users/admin/${id}`,{
            method: 'PUT',
            headers: {
    ...
    const handleDeleteUser = user =>{
        fetch(`https://secondhand-car-server-side.vercel.app/users/${user._id}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
